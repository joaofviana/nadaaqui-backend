-- Fix ambiguous column reference 'id' in nearby_places
-- The ORDER BY clause was referencing 'name' and 'distance_meters' without alias prefix
...
begin
  ...
  if p_bbox is not null and length(trim(p_bbox)) > 0 then
    ...
    v_min_lat := split_part(pbbox, ',', 2)::double precision;
